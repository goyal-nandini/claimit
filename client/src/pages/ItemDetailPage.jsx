...
const ItemDetailPage = () => {
  ...
  if (error) return (
    <div className="text-center py-24 animate-fade-in">
      ...
      <button onClick={() => navigate(-1)} className="mt-4 text-sm text-purple-600 hover:underline">Go back</button>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="flex justify-between items-start gap-3 mb-6">
            <h1 className="text-2xl font-extrabold text-gray-900 leading-tight" style={{ fontFamily: 'Plus Jakarta Sans' }}>
              {item.title}
            </h1>
            <div className="flex flex-col items-end gap-2 shrink-0">
              ...
              <span className={`text-xs font-medium px-3 py-0.5 rounded-full ${
                isResolved ? 'bg-gray-100 text-gray-500' : 'bg-purple-50 text-purple-700 border border-purple-100'
              }`}>
                {item.status}
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-5 mb-6 p-5 bg-gray-50 rounded-xl">
            ...
            <DetailRow icon="📅" label="Date" value={item.date ? new Date(item.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'} />
            <DetailRow icon="👤" label="Posted by" value={item.postedBy?.name} />
          </div>
          ...
          {isResolved && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl px-5 py-4 text-center">
              <p className="text-sm font-semibold text-gray-500">✅ This item has been resolved</p>
              <p className="text-xs text-gray-400 mt-1">Someone successfully claimed this item</p>
            </div>
          )}

          {!isResolved && isOwner && (
            <div className="bg-purple-50 border border-purple-100 rounded-xl px-5 py-4 text-center">
              <p className="text-sm font-semibold text-purple-700">🏷️ This is your posted item</p>
              <p className="text-xs text-purple-500 mt-1">You'll be notified when someone claims it</p>
            </div>
          )}
          ...
          {!isResolved && !isOwner && isLoggedIn && (
            <div>
              ...
              >
                ...
                ) : claimSuccess ? '✅ Claim Submitted' : '🙋 Claim This Item'}
              </button>
            </div>
          )}
        </div>
